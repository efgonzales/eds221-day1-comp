...
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE) #echo makes code show up for entire doc when knitted, no messages/warnings will show up for entire doc.
library(tidyverse)
library(here)
library(janitor)
library(skimr)
...
```{r}
toolik_weather <- toolik_weather %>% clean_names() # What does this do? Say it in words. : Take an object AND THEN clean the names into snake case...
...
toolik_temp_plot <- ggplot(data = toolik_weather, aes(x = as.factor(month), y = daily_air_temp_mean_c)) +
  ...
  labs(title = "Toolik Station daily air temperature",
       x = "Month",
       y = "Daily mean air temperatures (C)")
...
```{r}
ggsave(here::here("figs", "toolik_temp_plot.png"), height = 6, width = 7) #saves LAST created graph, unless you specify plot = "name of plot"
```
